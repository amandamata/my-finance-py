...
allocation_amounts = {asset: monthly_investment * percentage for asset, percentage in allocation_percentages.items()}

print("Monthly Allocation Amounts:")
for asset, amount in allocation_amounts.items():
    print(f"{asset.capitalize()}: R${amount:.2f}")

stock_allocation_amount = allocation_amounts['stocks']
total_stock_price = 0

for i, ticker in enumerate(tickers):
    current_data = yf.download(ticker, start=start_date, end=end_date)
...
for i, ticker in enumerate(tickers):
    current_data = yf.download(ticker, start=start_date, end=end_date)
    current_price = current_data['Close'].iloc[-1]
    allocated_amount = (current_price / total_stock_price) * stock_allocation_amount
    allocated_quantity = allocated_amount / current_price
    print(f"{ticker}: Allocate ${allocated_amount:.2f} for approximately {allocated_quantity:.2f} shares")
...
for i, ticker in enumerate(tickers):
    quantity = allocated_quantity if 'allocated_quantity' in locals() else 100
